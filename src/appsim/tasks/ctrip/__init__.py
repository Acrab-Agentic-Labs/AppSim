...
CTRIP_TASKS = AppTasks(
    ...
    task_items=[
        TaskItem(instruction="进入酒店预订。", verify_func=check_click_hotel),
        TaskItem(instruction="进入机票预订。", verify_func=check_click_flight),
        TaskItem(instruction="看一下火车票预订。", verify_func=check_click_train),
        TaskItem(instruction="看一下消息列表。", verify_func=check_click_message),
        TaskItem(instruction="看看我的行程。", verify_func=check_click_itinerary),
        TaskItem(instruction="进入我的页面。", verify_func=check_click_profile),
        TaskItem(instruction="给我看看上海的酒店列表。", verify_func=check_hotel_search_chengdu),
        TaskItem(
            instruction="帮我找入住时间10月20日，退房时间10月21日的酒店列表。", verify_func=check_hotel_search_dates
        ),
        TaskItem(instruction="帮我找间数1、成人数1、儿童数1，的酒店列表。", verify_func=check_hotel_search_guests),
        TaskItem(instruction='帮我找出发地 "广州"、目的地 "深圳"的航班列表。', verify_func=check_flight_search_gz_sz),
        TaskItem(instruction="找出日期10月22日的航班列表。", verify_func=check_flight_search_date),
        TaskItem(instruction="找出公务/头等舱的航班列表。", verify_func=check_flight_search_cabin),
        TaskItem(instruction='找出发地 "北京"、目的地 "上海"的火车车次列表。', verify_func=check_train_search_bj_sh),
        TaskItem(instruction="找到日期10月24日的火车车次列表。", verify_func=check_train_search_date),
        TaskItem(instruction="找到能用学生票的车次列表。", verify_func=check_train_search_student),
        TaskItem(
            instruction="找出入住时间10月22日，退房时间10月23日的北京酒店列表。", verify_func=check_hotel_search_beijing
        ),
        TaskItem(
            instruction="找出入住时间今天，退房时间明天，间数2，成人数2，儿童数0的上海酒店列表。",
            verify_func=check_hotel_search_shanghai,
        ),
        TaskItem(instruction="出发地北京，目的地深圳，10月25号，看看有啥航班。", verify_func=check_flight_search_bj_sz),
        TaskItem(
        ...
        TaskItem(instruction="帮我检索上海所有酒店中最低的价格。", verify_func=check_hotel_search_shanghai_min_price),
        TaskItem(
        ...
        TaskItem(instruction="订5张10月20日深圳到北京的火车票，5小时内到达。", verify_func=check_booking_batch_train),
        TaskItem(
            instruction="订5张10月20日广州到北京的火车票，再订北京 3 家不同的酒店。",
            verify_func=check_booking_complex_batch,
        ),
        TaskItem(instruction="订10月25日北京飞上海的6张经济舱机票。", verify_func=check_booking_batch_flight),
    ],
)

# 为了向后兼容，保留 ALL_INSTRUCTION
ALL_INSTRUCTION = [{"instruct": item.instruction, "fun": item.verify_func} for item in CTRIP_TASKS.task_items]
