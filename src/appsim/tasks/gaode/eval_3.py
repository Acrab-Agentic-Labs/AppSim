#!/usr/bin/env python3
"""
Android App Food Search Log Verification Script

This script verifies that the "搜索美食" action was correctly recorded
...
def verify_last_log3(result=None, device_id=None):
    """
    Verify the last log entry in 3_food_search_history.json matches the expected action.
    ...
    try:
        ...
        cmd.extend(["exec-out", "run-as", "com.example.GaoDe", "cat", "files/3_food_search_history.json"])
        ...
        expected_action = "搜索美食"
        actual_action = last_record["action"]
        if actual_action == expected_action:
            print("PASS: Food search action verification successful")
...
if __name__ == "__main__":
    # Verify the expected operation: "搜索美食"
    success = verify_last_log3()
