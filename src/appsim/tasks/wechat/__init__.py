...
WECHAT_TASKS = AppTasks(
    ...
    task_items=[
        ...
        TaskItem(
            instruction='发信息到工作群，说"GUI Agent最近很火，我觉得挺有意思的"', verify_func=Task2_MessageSendCheck
        ),
        TaskItem(
            instruction="查看北京大学李老师发给我的信息，看看参会的听众人数是多少，我好提前去订会议室。告诉我数字即可。",
            verify_func=Task3_number_count,
        ),
        TaskItem(instruction="看看我有多少个微信好友。告诉我数字即可。", verify_func=Task4_number_count),
        TaskItem(
...
ALL_INSTRUCTION = [{"instruct": item.instruction, "fun": item.verify_func} for item in WECHAT_TASKS.task_items]
