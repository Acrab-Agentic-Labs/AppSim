...
def validate_task_sixteen(result=None, device_id=None):
    ...
    subprocess.run(cmd, stdout=open("task_sixteen_log.txt", "w"))

    try:
        with open("task_sixteen_log.txt", "r", encoding="utf-8") as f:
            content = f.read()
    except:
        return False
